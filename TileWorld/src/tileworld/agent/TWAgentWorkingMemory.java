 ...
 *         want to develop an entirely new memory system or extend/modify this
 *         one.
 * 
 *         The memory is supposed to have a probabilistic decay, whereby an
 *         element is removed from memory with a probability proportional to the
 *         length of time the element has been in memory. The maximum length of
 *         time which the agent can remember is specified as MAX_TIME. Any
 *         memories beyond this are automatically removed.
 */
public class TWAgentWorkingMemory {
	...
	/*
	 * This was originally a queue ordered by the time at which the fact was
	 * observed. However, when updating the memory a queue is very slow. Here we
	 * trade off memory (in that we maintian a complete image of the map) for
	 * speed of update. Updating the memory is a lot more straightfoward.
	 */
	...
	static private List<Int2D> spiral = new NeighbourSpiral(
			Parameters.defaultSensorRange * 4).spiral();
	...
	public TWAgentWorkingMemory(TWAgent moi, Schedule schedule, int x, int y) {
		...
		this.memoryGrid = new ObjectGrid2D(me.getEnvironment().getxDimension(),
				me.getEnvironment().getyDimension());
	}
	 ...
	 *
	 * @param sensedObjects
	 *            bag containing the sensed objects
	 * @param objectXCoords
	 *            bag containing x coordinates of objects
	 * @param objectYCoords
	 *            bag containing y coordinates of object
	 * @param sensedAgents
	 *            bag containing the sensed agents
	 * @param agentXCoords
	 *            bag containing x coordinates of agents
	 * @param agentYCoords
	 *            bag containing y coordinates of agents
	 */
	public void updateMemory(Bag sensedObjects, IntBag objectXCoords,
			IntBag objectYCoords, Bag sensedAgents, IntBag agentXCoords,
			IntBag agentYCoords) {
		// reset the closest objects for new iteration of the loop (this is
		// short
		// term observation memory if you like) It only lasts one timestep
		closestInSensorRange = new HashMap<Class<?>, TWEntity>(4);

		// must all be same size.
		assert (sensedObjects.size() == objectXCoords.size() && sensedObjects
				.size() == objectYCoords.size());
		this.memoryGrid.clear();
		// me.getEnvironment().getMemoryGrid().clear(); // THis is equivalent to
		// only having sensed area in memory
		this.decayMemory(); // You might want to think about when to call the
							// decay function as well.
		for (int i = 0; i < sensedObjects.size(); i++) {
			...
			objects[objectXCoords.get(i)][objectYCoords.get(i)] = new TWAgentPercept(
					o, this.getSimulationTime());
	...
	public void updateMemory(TWEntity[][] sensed, int xOffset, int yOffset) {
		for (int x = 0; x < sensed.length; x++) {
			for (int y = 0; y < sensed[x].length; y++) {
				objects[x + xOffset][y + yOffset] = new TWAgentPercept(
						sensed[x][y], this.getSimulationTime());
			}
		}
	}
	...
	public void decayMemory() {
		...
		// if(currentMemory!=null && currentMemory.getT() <
		// schedule.getTime()-MAX_TIME){
	 ...
	 *
	 * @param x
	 *            coordinate from which to check for tiles
	 * @param y
	 *            coordinate from which to check for tiles
	 * @param threshold
	 *            how recently we want to have see the object
	 * @param type
	 *            the class of object we're looking for (Must inherit from
	 *            TWObject, specifically tile or hole)
	 * @return
	 */
	private TWObject getNearbyObject(int sx, int sy, double threshold,
			Class<?> type) {
		...
		for (Int2D offset : spiral) {
			...
			if (me.getEnvironment().isInBounds(x, y) && objects[x][y] != null) {
				...
				if (type.isInstance(o)) {// if it's not the type we're looking
											// for do nothing
	...
	private void updateClosest(TWEntity o) {
		...
		if (closestInSensorRange.get(o.getClass()) == null
				|| me.closerTo(o, closestInSensorRange.get(o.getClass()))) {
	...
	public boolean isCellBlocked(int tx, int ty) {
		...
		if (this.memoryGrid.get(tx, ty)== null) {
			return false;
		}

		TWEntity e = (TWEntity) this.memoryGrid.get(tx, ty);
		// is it an obstacle?
		return (e instanceof TWObstacle);
	}
